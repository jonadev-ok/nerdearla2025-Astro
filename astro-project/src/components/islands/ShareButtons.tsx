...
export default function ShareButtons({ url}: Props) {
  ...
  const twitterURL = `https://x.com/intent/post?text=url=${encodeURIComponent(url)}}`;
  ...
  return (
    <div class="flex w-full justify-center gap-4 mt-4">
      ...
      >
        <span>Compartir en</span> <img class="w-4" src="/logos/Xspace.svg" alt="x-space" loading="lazy"/>
      </a>
      <a
        href={linkedinURL}
        target="_blank"
        ...
        class="px-3 py-2 flex justify-around items-center  bg-blue-500 text-white rounded w-40"
      >
        <span>Compartir en</span> <img class="w-4" src="/logos/Linkedin.svg" alt="linkedin" loading="lazy"/>
      </a>
      <button
        onClick={copyLink}
